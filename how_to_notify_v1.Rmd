...
Motivation: sometimes we have a long-running R job that we leave alone while we do other things, and when this happens, it would be nice if R notified us when it was done. Below we'll walk through how to automate having R send you an: email, text message, `Slack` message, or `Microsoft Teams` message.
